...
class LeoRegexpApplyTool:
    ...
    def apply(self, filePath: str, *regexps:[LeoRegexp]):
    ...
    def _doRegProcess(self, regexp:LeoRegexp, inputText:str):
        ...
        for regexpMatch in re.finditer(regexp.match, outputText, re.DOTALL):
            if(regexpMatch != None):
                outputText = self._replaceOrNot(regexp, regexpMatch, outputText)
        return outputText

    def _replaceOrNot(self, regexp:[LeoRegexp], regexpMatch:[re.Match], inputText:str):
        outputText = copy.deepcopy(inputText)
        if(regexp.replace != None):
            outputText = self._generateReplaceText(regexp, regexpMatch, outputText)
        return outputText

    def _generateReplaceText(self, regexp:LeoRegexp, regexpMatch:[re.Match], inputText: str):
        replaceWithThisStr = self._parseDollerSignToTextStr(regexp, regexpMatch)
    ...
    def _parseDollerSignToTextStr(self, regexp:LeoRegexp, regexpMatch:[re.Match]):
        finishParsedReplaceStr = copy.deepcopy(regexp.replace)
        howManyGroup = len(regexpMatch.groups())
