...
class LeoRegexpApplyTool:
    ...
    def _doRegProcess(self, regexp: LeoRegexp, inputText: str):
        ...
        for regexpMatch in re.finditer(regexp.match, outputText, re.DOTALL):
            self._geneateOutputText(regexp, regexpMatch, outputText)
        return outputText

    def _geneateOutputText(self, regexp: LeoRegexp, regexpMatch: List[re.Match], inputText: str):
        outputText = inputText
        if(regexpMatch != None):
    ...
    def _replaceTextOrNot(self, regexp: LeoRegexp, regexpMatch: List[re.Match], inputText: str):
        outputText = inputText
